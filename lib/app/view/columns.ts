...
import {ActionColumnViewCtx} from '../columns'; 
//@ts-ignore: 6192
import {
Maybe as __Maybe,
...
export class ActionColumnView  implements __wml.View {
   ...
   constructor(__context: ActionColumnViewCtx  ) {
       ...
       this.template = (__this:__wml.Registry) => {
           ...
           return __this.node('td', <__wml.Attrs>{wml : { 'id' : __context.id  },'class': __context.className}, [

        __this.widget(new Button({ww : { 'text' : 'Run' ,'onClick' : __context.onClick  }}, [

        
     ]),<__wml.Attrs>{ww : { 'text' : 'Run' ,'onClick' : __context.onClick  }})
     ]);
